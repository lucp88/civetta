            ...
            if ($action === 'batches') {
                ...
                $sql = "SELECT i.id, i.name, i.category, i.unit, i.brand_name, i.parent_id,
                        ...
                        (SELECT price_per_kg FROM ingredient_batches
                         ...
                         ORDER BY COALESCE(thd_date, '9999-12-31') ASC, purchase_date ASC LIMIT 1) as fifo_price_per_kg
                        FROM ingredients i
...
function consumeIngredient($pdo, $ingredientId, $quantityGrams, $orderId = null) {
    ...
    try {
        ...
        foreach ($batches as $batch) {
            ...
            $stmt = $pdo->prepare("UPDATE ingredient_batches SET quantity_remaining = quantity_remaining - ?, is_open = ? WHERE id = ?");
            $stmt->execute([$useFromBatch, $newRemaining > 0 ? 1 : 0, $batch['id']]);
