...
switch ($action) {
    case 'check':
        ...
        $displayName = null;
        if ($authenticated && isset($_SESSION['admin_user'])) {
            try {
                $stmt = $pdo->prepare("SELECT display_name FROM users WHERE username = ?");
                $stmt->execute([$_SESSION['admin_user']]);
                $row = $stmt->fetch();
                $displayName = ($row && $row['display_name']) ? $row['display_name'] : $_SESSION['admin_user'];
            } catch (PDOException $e) {
                $displayName = $_SESSION['admin_user'];
            }
        }
        echo json_encode([
            'success' => true,
