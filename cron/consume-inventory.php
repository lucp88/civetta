...
function consumeIngredient($pdo, $ingredientId, $quantityGrams, $orderId = null) {
    ...
    try {
        ...
        foreach ($batches as $batch) {
            ...
            $updStmt = $pdo->prepare("UPDATE ingredient_batches SET quantity_remaining = quantity_remaining - ?, is_open = ? WHERE id = ?");
            $updStmt->execute([$useFromBatch, $newRemaining > 0 ? 1 : 0, $batch['id']]);

            $insStmt = $pdo->prepare("
                INSERT INTO inventory_consumption (ingredient_id, batch_id, order_id, quantity_consumed, cost)
                VALUES (?, ?, ?, ?, ?)
            ");
            $insStmt->execute([$ingredientId, $batch['id'], $orderId, $useFromBatch, $costForBatch]);
