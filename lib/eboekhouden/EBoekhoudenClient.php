 ...
 *
 * Handles authentication, invoicing, relations, mutations and payment matching.
 */
...
class EBoekhoudenClient {
    ...
    private function log($message) {
        $logFile = __DIR__ . '/../../api/webhook-debug.log';
...
function getEBoekhoudenClient($pdo) {
    ...
    $settings = getSettings($pdo, ['facturatie_systeem', 'eboekhouden_api_token']);

    if ($settings['facturatie_systeem'] !== 'eboekhouden') {
        return null;
    }
    if (!$settings['eboekhouden_api_token']) {
        return null;
    }
